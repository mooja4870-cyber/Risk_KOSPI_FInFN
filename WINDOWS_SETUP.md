...
현재 Mac에서 정상 동작 중인 **KOSPI '금융투자' 수급 분석 앱(Risk_KOSPI)**을 윈도우(Windows) PC에서 완전히 똑같이 구현하기 위한 A to Z 가이드북입니다. 기초 환경 세팅부터 실행까지 따라오기만 하면 완벽히 똑같은 대시보드를 띄울 수 있도록 중학생들도 이해하기 쉽게 작성했습니다.
